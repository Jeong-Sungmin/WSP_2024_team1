...
async function processFairytaleDataExpert(receivedData, index) {
  ...
  const model = genAI.getGenerativeModel({
    ...
    systemInstruction:
      '지금부터 동화를 제작할거야. 동화는 한국어로 작성할 것. 동화에 대한 속성을 말해줄게. 만약 속성이 비어있다면, 네가 가장 적당해보이는 값으로 대신 채워넣어서 동화를 제작해줘.\n동화 제목과 내용을 제외한 다른 말은 하지 말 것. 동화의 방식은 맨 윗 줄에 동화의 제목을 작성하고, 이후 6개의 문단으로 나뉘는 본문을 작성할 것. 각 문단은 180자를 넘지 않도록 분량을 조절하고, 나레이터와 등장인물들의 대사가 반복되는 형태로 만들 것.(나레이터의 대사는 일반문(쌍따옴표를 사용하지 않는 문장)으로 작성하고, 등장인물의 대사가 나오기 전, "~~~가 말했다."라는 나레이터의 대사를 넣고, 등장인물의 대사는 모두 쌍따옴표를 넣어서 작성할 것.) 문단을 나눌 때 문단을 구분하는 요소는 block_1, block_2 ... 를 문단 위에 작성할 것.(문단을 나누는 이유는, 동화를 게시하는 사이트에서 6페이지 짜리 동화를 만들라고 했기 때문에, 페이지를 구분하기 위함.) 따라서 동화는 (제목) block_1 (내용) block_2 ... 형식으로 만들어질 것임.',
  });
